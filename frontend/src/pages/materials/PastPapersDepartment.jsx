...
import {
	PAST_PAPER_DATA,
	PAST_PAPER_DEPARTMENTS,
	PAST_PAPER_YEARS,
} from "./pastPapersData";

const PastPapersDepartment = () => {
	...
	const departmentInfo = useMemo(
		() =>
			PAST_PAPER_DEPARTMENTS.find(
				(item) => item.code.toLowerCase() === departmentCode.toLowerCase(),
			),
		[departmentCode],
	);

	const [selectedYear, setSelectedYear] = useState(PAST_PAPER_YEARS[0]);
	...
	const yearModules = PAST_PAPER_DATA[departmentInfo.code][selectedYear];
			...
			<main className="relative mx-auto max-w-6xl px-4 pb-16 pt-28 md:px-8">
				<div className="rounded-3xl border border-white/60 bg-white/80 p-6 shadow-lg backdrop-blur-xl md:p-8">
					...
					<p className="mt-5 text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">
						Materials / Past Papers / {departmentInfo.code}
					</p>
					<h1 className="mt-2 text-3xl font-bold text-slate-900 md:text-4xl">
						{departmentInfo.code} Past Papers
					</h1>
					...
					<div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
						{PAST_PAPER_YEARS.map((yearLabel) => {
...
export default PastPapersDepartment;
